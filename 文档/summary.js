...
******************************************************************************
  ...
  四、Vue 2.0 的更新机制(虚拟dom、diff算法)
    **************************************************************************
      ...
      5、特殊情况当 1,2,3,4 的情况都不符合的时候就会执行, 在oldVnodes里面寻找跟newStartVnode一样
        的节点然后位移到oldStartVnode, 若没有找到在就oldStartVnode创建一个

      执行过程是一个循环,在每次循环里,只要执行了上述的情况的五种之一就会结束一次循环
      循环结束的收尾工作:直到oldStartIdx>oldEndIdx || newStartIdx>newEndIdx(代表旧节点或者新节
      点已经遍历完)
    **************************************************************************
      为什么要使用虚拟DOM

      1、虚拟DOM可以维护程序的状态,跟踪状态变化
        通过比较前后两次状态差异更高效的更新真实DOM
      ...
      3、真实DOM的属性很多，创建DOM节点开销很大
        虚拟DOM只是普通JavaScript对象，描述属性并不需要很多，创建开销很小

      4、复杂视图情况下提升渲染性能(操作dom性能消耗大,减少操作dom的范围可以提升性能)，当然如果是简答的
        节点直接创建更快
...
************************************************************************************************************
  * Vue3 里为什么要用 Proxy API 替代 defineProperty API
    ...
    Vue2 中的响应式需要对 data 中的属性做了递归遍历，为每个属性以及属性所有子属性设置了 getter、setter。
    这也是 Vue2 只能对 data 中预定义过的属性做出响应的原因
    因为要递归遍历所有的data对象（包括data属性、计算属性、监听属性），肯定会造成很大的性能消耗。
      * this.arr[0] = 55  arr = [2, 5] 不更新页面  arr = {0:2, 1:5} 更新页面
      * this.obj.b = 3  obj = {a: 3} 不更新页面  obj = {a: 2, b: 5} 更新页面

    Proxy API的监听是针对整个对象的，相当于在对象的操作之前加了一层拦截，对这个对象的所有操作都会通过代理
    来实现，就是实现了按需响应式，这样就可以减少性能消耗。
